...
for(b in seq_len(B)) {
  ...
  ## step one: bootstrap sample
  idx <- sample(seq_len(n), n, replace = TRUE)
  x_star <- law[idx, ]
  
  r[b] <- cor(x_star$LSAT, x_star$GPA)
}

ggplot() +
  geom_histogram(aes(r)) +
  geom_vline(aes(xintercept = theta), colour = "blue") +
  geom_vline(aes(xintercept = theta_hat), colour = "red", lty = 2)

## estimate sd(theta_hat)
sd(r)

## estimate bias(theta_hat)
mean(r) - theta_hat