...
# f_X(x) = 3x^2, 0 <= x \<= 1
F_inv <- function(u) {
  if(any(u < 0 | u > 1)) stop("Pass through values between 0 and 1.")
  u^(1/3)
}
...
## check we are matching the theoretical density
x_grid <- seq(0, 1, length.out = 300) ## x values to plot the density at

ggplot() +
  geom_histogram(aes(x, y = after_stat(density))) + ## y = after_stat(density) places the histogram on the density scale
  geom_line(aes(x_grid, 3*x_grid^2), colour = "red")
...
## ------------------------------------------------------------------------
ggplot() +
  geom_histogram(aes(samples, y = after_stat(density)), bins = 50) +
