...
ggplot(mpg, aes(cty, hwy)) +
  geom_jitter(aes(colour = drv, shape = drv), size = 5)
...
ggplot(mpg) +
  geom_histogram(aes(hwy), bins = 60)
...
ggplot(data = mpg) +
  ...
  geom_point(aes(drv, hwy), alpha = .5)
...
ggplot(mpg) +
  geom_point(aes(cty, hwy, colour = as.character(year), shape = as.character(year), size = as.character(year))) +
