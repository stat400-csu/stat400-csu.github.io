...
# strong, positive, linear relationship btw hwy and cty
...
ggplot(data = mpg, aes(cty, hwy)) +
  geom_point(mapping = aes(colour = drv, shape = drv), size = 5)
...
ggplot(data = mpg) +
  geom_boxplot(mapping = aes(drv, hwy)) +
  # geom_point(mapping = aes(drv, hwy), alpha = .5)
...
ggplot(data = mpg) +
  geom_histogram(mapping = aes(hwy), bins = 20) +
  facet_wrap(~ drv)
...
ggplot(data = mpg, mapping = aes(cty, hwy)) +
  geom_point(aes(color = class, shape = drv, size = displ)) +
  facet_wrap(~drv)
...
ggplot(data = mpg) +
  geom_histogram(mapping = aes(hwy, fill = as.factor(cyl), group = as.factor(cyl)), binwidth = 5, position = "dodge") +
  facet_wrap(~ drv)
