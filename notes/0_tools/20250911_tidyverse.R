...
recent_grads |> filter(Total > 100000 | Major_category == "Computers & Mathematics")
...
math_grads |> select(-Major, -Rank)
math_grads |> select(-(College_jobs:Low_wage_jobs))
...
nfl |>
  group_by(Position) |>
  summarise(avg_sal = mean(Salary, na.rm = TRUE), sd_sal = sd(Salary, na.rm = TRUE)) -> position_summary
...
table4a |>
  pivot_longer(-country, names_to = "year", values_to = "cases") |>
  mutate(year = as.numeric(year)) -> table4a_tidy

table4b |>
  pivot_longer(-country, names_to = "year", values_to = "population") |>
  mutate(year = as.numeric(year)) -> table4b_tidy
...
world_bank_pop |>
  pivot_longer(-c(country, indicator), names_to = "year", values_to = "values") |> 
