...
for(j in 1:length(epsilon)) {
  ...
  y <- rep(NA, m)
  for(i in seq_len(m)) {
    x <- r_noisy_normal(n, epsilon[j]) # 1. generate data
    t_star <- skew(x) # 2. compute the statistic
    y[i] <- abs(t_star) > crit_val # 3. perform the test
  }
...
ggplot() +
  ...
  geom_ribbon(aes(x = epsilon, ymin = empirical_pwr - empirical_se, ymax = empirical_pwr + empirical_se), alpha = .5) +
