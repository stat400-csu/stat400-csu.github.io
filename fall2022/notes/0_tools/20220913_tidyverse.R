...
# 1. Yes, seems tidy -- each observation (player) has its own row and each column is a measurement (variable) about that observation.
# 2. 
head(world_bank_pop)

# we have observational units (country/year) condensed into a single row (rather than having each their own row)
# we have multiple measurements in the same column

# 3.
world_bank_pop |>
  pivot_wider(names_from = c("indicator"), values_from = `2000`:`2017`) |>
  pivot_longer(-country, names_to = "year_indicator", values_to = "value")

# this is getting complicated, let's try the otherway
